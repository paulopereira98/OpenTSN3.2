The quartus version used in the TSN network card example project is Quartus Prime Standard Edition 19.1, the FPGA model used is Intel Arria10:10AX048H2F34E2SG, and a total of 16 ip core files are used in the hardware logic source code (including core code, device-related code and interface-related code). , the user can generate the corresponding IP core according to the following IP core configuration parameter description information when building the TSN network card example project, and place the generated files of all IP cores (including the QSYS file) in the current path.
...
(2) IP core: altera_eth_tse (after generating the three-speed Ethernet IP core, two files need to be replaced, see ./sgmii_pcs_revise_note for details)
    ...
    Component: 10AX048H2F34E2SG
    Use internal fifo: deseclect (do not check Use internal fifo)
    Number of ports : 4
